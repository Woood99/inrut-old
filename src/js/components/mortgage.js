...
export const mortgageTwo = () => {
    const containerAdd = document.querySelector('.object-calc-mort--addd');
    ...
    if (containerAdd) {
        ...
        if (meternalCapital) {
            ...
            function updateFee() {
                setTimeout(() => {
                    const capitalValue = capitalInput.value.replace(/\s/g, '');
                    meternalCapitalSlider.set(+capitalValue);
                }, 100);
            }
            ...
            function validate() {
                ...
                const sum = Number(capitalInput.value.replace(/\s/g, ''));
                if (sum > Number(priceObject.dataset.value) * 90 / 100) {
                    result = false;
                }
