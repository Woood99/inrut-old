...
export const bankOfferSecond = () => {
    const items = document.querySelectorAll('.bank-offer-second');
    if (items.length >= 1) {
        items.forEach(container => {
            const additional = container.querySelector('.bank-offer-second__block');
            const additionalItems = container.querySelectorAll('.bank-offer-second__additional-item');

            const infoItems = container.querySelectorAll('.bank-offer-second__info-item');
            const bid = infoItems[0].querySelector('[data-bank-offer-default-prc]');
            const monthPaymentTop = infoItems[1].querySelector('div > span');

            const selector = container.querySelector('.bank-offer-second__selector');
            if (selector) {
                const btn = selector.querySelector('.bank-offer-second__selector-top');
                const closeBtn = selector.querySelector('.bank-offer-second__close-item');
                btn.addEventListener('click', () => {
                    if (!selector.classList.contains('_active')) {
                        selector.classList.add('_active');
                        if (additional) additional.removeAttribute('hidden');
            ...
            if (selectorList && additionalItems.length > 0) {
                ...
                additionalItems.forEach(item => {
                    const btn = item.querySelector('.bank-offer-second__additional-item__btn');
                    const descr = item.querySelector('.bank-offer-second__additional-item__descr');
                    moreDescr(btn, descr);
                    const toggle = item.querySelector('.toggle-checkbox');
                    const toggleInput = toggle.querySelector('input');
                    const span = toggle.previousElementSibling;
            ...
            const addInfo = container.querySelector('.bank-offer-second__add-info');
            if (addInfo) {
                const btn = addInfo.querySelector('.bank-offer-second__add-info-top');
                const selectorContent = addInfo.querySelector('.bank-offer-second__add-info-content');
                const closeBtn = addInfo.querySelector('.bank-offer-second__close-item');
                btn.addEventListener('click', () => {
                    if (!addInfo.classList.contains('_active')) {
                        addInfo.classList.add('_active');
                        selectorContent.removeAttribute('hidden');
